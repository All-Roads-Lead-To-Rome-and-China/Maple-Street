...
vi.mock("../../components/InventoryList", () => ({
  default: () => <div data-testid="inventory-list">Inventory List</div>,
}));
vi.mock("../../components/AppointmentScheduling", () => ({
  default: () => (
    <div data-testid="appointment-scheduling">Appointment Scheduling</div>
  ),
}));
vi.mock("../../components/StaffScheduling", () => ({
  default: () => <div data-testid="staff-scheduling">Staff Scheduling</div>,
}));
vi.mock("../../components/Invoices", () => ({
  default: () => <div data-testid="invoices">Invoices</div>,
}));
...
describe("StaffDashboard Component", () => {
  ...
  it("navigates between tabs", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Customer Records" })
    ).toBeInTheDocument();
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Inventory Management" })
    );
    expect(screen.getByTestId("inventory-list")).toBeInTheDocument();

    // Click on Invoices
    fireEvent.click(
      screen.getByRole("button", { name: "Invoices & Payments" })
    );
    expect(screen.getByTestId("invoices")).toBeInTheDocument();

    // Click on Compliance
    fireEvent.click(
      screen.getByRole("button", { name: "Compliance & Safety" })
    );
    expect(
      screen.getByRole("heading", { name: "Compliance & Safety" })
    ).toBeInTheDocument();
